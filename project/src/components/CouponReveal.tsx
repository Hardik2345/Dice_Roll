...
export function CouponReveal({
  shopUrl = "dice-roll-app.myshopify.com",
}: CouponRevealProps) {
  ...
  const handleShopNow = () => {
    ...
    // If we have a Shopify URL from the backend, use it
    if (coupon?.shopifyUrl) {
      window.open(coupon.shopifyUrl, "_blank");
    } else if (shopUrl === "#") {
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
      <div className="text-center max-w-sm mx-auto">
        ...
        <div className="relative mb-8">
          <div className="absolute -top-8 left-1/4 text-white text-2xl animate-bounce">
            ✨
          </div>
          <div className="absolute -top-6 right-1/4 text-white text-xl animate-pulse delay-300">
            ✨
          </div>
          <div className="absolute top-4 -left-8 text-white text-lg animate-bounce delay-500">
            ✨
          </div>
          <div className="absolute top-2 -right-6 text-white text-xl animate-pulse delay-700">
            ✨
          </div>
        </div>
        ...
        >
          ...
          <div className="relative w-64 h-80 mx-auto mb-8">
            ...
            {/* Coupon Paper */}
            <div
              className={`absolute inset-4 bg-white rounded-lg shadow-lg transition-all duration-1000 ${
