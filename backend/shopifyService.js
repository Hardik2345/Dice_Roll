...
class ShopifyService {
  ...
  async createPriceRule(discountPercentage, code) {
    ...
    const priceRule = await this.makeRequest(
      "/price_rules.json",
      "POST",
      priceRuleData
    );
  ...
  async createDiceRollDiscount(diceResult, userName, userMobile) {
    ...
    // Generate unique code using mobile number
    const code = `DICE${percentage}_${userMobile}`;
  ...
  async createDiscountWithExpiry(
    diceResult,
    userName,
    userMobile,
    expiryDays = 30
    ...
    try {
      const priceRule = await this.makeRequest("/price_rules.json", "POST", {
